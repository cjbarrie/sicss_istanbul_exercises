"""Module 1: Prompt Stability -- with the REAL `promptstability` package
=============================================================================

This uses the actual `promptstability` package (Barrie, Palaiologou &
Toernberg, 2024) -- not a mock -- pointed at a small model running locally
via Ollama. No API key, no cost, and (with temperature=0 + a fixed seed)
fully reproducible, because you control the exact model weights and version.

Two things are measured, exactly as in Paper 1:
  - INTRA-PSS: run the *same* prompt several times. How consistent is the
    model with itself? (`psa.intra_pss`)
  - INTER-PSS: run several *reworded* versions of the prompt once each. How
    consistent is the model across wording changes? (`psa.manual_inter_pss`)

A NOTE ON WHAT TO EXPECT
------------------------
This calls a live local model with no fixed random seed for the annotation
calls themselves (only `ollama_repeated_check.py`'s temperature=0+seed run is
pinned). That means YOUR numbers will differ from anyone else's, and from
one run to the next on the same machine -- not because anything is wrong,
but because you're looking at real model behavior on a small sample, not a
canned demo. There is no single "correct" curve here. See the README's
"What to expect" section before you conclude your run looks "wrong."

SETUP (see README.md for full detail)
--------------------------------------
1. Install Ollama (https://ollama.com) and pull a small model:
       ollama pull llama3.2:3b
2. `promptstability` requires Python 3.8-3.10 (NOT 3.11+). Create a matching
   virtual environment, then:
       pip install promptstability ollama
3. Run this script from inside `module_1_prompt_stability/`:
       python starter.py
"""
